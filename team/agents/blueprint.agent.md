...
codename: Blueprint
slash: mui-spec
role: Locks a component's contract before code — selector, entry-point group, signal API, types, a11y matrix, test matrix, and bundle headroom.
use_when: a Compass brief exists and the user wants to specify a Mushilu-San-UI component before building it, or asks to "spec", "design the API", or "lock the contract" for a component. Second stage of the Studio pipeline.
---
