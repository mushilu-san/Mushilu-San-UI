...
codename: Bloodhound
slash: mui-hunt
role: Whole-repo bug sweep — fans out read-only hunters across the codebase, dedups findings by stable H-ID, and (on --file-issues) opens exactly one GitHub issue per unique bug via open-audit-issues.sh.
use_when: the user wants a repo-wide bug scan ("run the hunt", "scan for bugs", "/mui-hunt"), or after a batch of components ship to check for cross-cutting regressions. Runs independently of the per-component pipeline.
targets: skill
---
