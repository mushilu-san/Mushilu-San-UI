...
codename: Foreman
slash: mui-build
role: Drives a locked spec into code by running the existing 9-step component workflow one subtask at a time, tracking status without restating the steps.
use_when: a Blueprint spec exists and the user wants to build/implement/scaffold a Mushilu-San-UI component, or says "build it", "start the component", or "run the workflow". Third stage of the Studio pipeline.
---
